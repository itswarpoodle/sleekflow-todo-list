...
**Dependency enforcement is stricter than the stated transition.** The brief explicitly prevents a blocked TODO from entering `IN_PROGRESS`, but says nothing about moving it directly to `COMPLETED`. Allowing that shortcut would bypass the dependency entirely, so a blocked TODO cannot enter either state. Only the recorded `COMPLETED` status satisfies a dependency; archiving or deleting an incomplete prerequisite does not convert it into completed work. A prerequisite completed before soft deletion remains satisfied because its retained status preserves that history. A blocked TODO may still be edited while `NOT_STARTED`, archived, or deleted so users can correct or retire work that cannot proceed. Existing soft-deleted dependency links remain attached during permitted edits instead of making the dependent TODO uneditable. I also reject missing dependencies, self-dependencies, and transitive cycles because accepting them would create an invalid graph.
...
Third, I would add an audited restore and dependency-recovery workflow. The current rule deliberately keeps a TODO blocked when an incomplete prerequisite is deleted, while still allowing that TODO to be edited while not started, archived, or deleted. A production product should let an authorized user restore the prerequisite or explicitly remove or replace the dependency with a recorded reason.
